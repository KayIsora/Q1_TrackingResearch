"""NEW AUDITABLE ADAPTER EXTENSION for the Stage-4A-R reproduction.

The exact previous adapter is preserved byte-for-byte as
``artifacts/stage4A_reproduction/previous_adapter_exact.py``. This extension
keeps its tracking and metric logic and exposes the requested runtime-mode
switch. The sequence set remains fixed and was selected only from local
image/ground-truth completeness and sequence length. This is not a full OTB
benchmark runner.
"""
...
# Adapter contract retained from the previous script: initialize receives only
# init_bbox; each subsequent track call receives an empty info dictionary; no
# previous_output is propagated by this direct adapter.
ADAPTER_INFO_CONTRACT = (
    "initialize info contains init_bbox only; frame info is {}; "
    "previous_output is not propagated"
)
...
def parse_args():
    ...
    parser.add_argument(
        "--runtime-mode",
        choices=("deterministic", "default"),
        default="deterministic",
        help="deterministic Stage-4 characterization or unforced local defaults",
    )
...
def main():
    ...
    if args.runtime_mode == "deterministic":
    ...
    report = {
        ...
        "seed": args.seed if args.runtime_mode == "deterministic" else None,
        ...
        "adapter_info_contract": ADAPTER_INFO_CONTRACT,
        "results": results,
