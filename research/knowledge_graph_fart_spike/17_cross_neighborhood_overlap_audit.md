...
|---|---|---|---|---|---|---|
| Adaptive Target-Oriented Tracking | Target-aware selection resembles IFAS's target-relevant token retention | Suppressing distractor/background evidence relates to SpikeTrack's reported fine-grained/similar-object weakness | One-stream tracker with untied positional encoding and Auto-Mask Learner that masks ineffective non-target search information | Functional within target filtering, but no demonstrated structural reduction or SpikeTrack-compatible preservation signal; overlap is contextual | `CONTEXTUAL_REFERENCE` | No |
| CETTrack / SuperSBT | Single-branch joint relation modeling provides context for one-stream efficiency design | Correlation depth and multi-level template-search relation are relevant only as an alternative to the six-MRM design | SBT/SuperSBT embed cross-image correlation through multiple backbone layers, with hierarchical/local relation modeling and a tracking head | Mechanism is real, but its connection to the favored fixed reduction + task-facing preservation family is bibliographic/framework-level | `CONTEXTUAL_REFERENCE` | No |
...
Evidence: Adaptive Target-Oriented Tracking [E29]; CETTrack/SBT/SuperSBT [E30]; LoReTrack [E15]; DyTrack [E08, E13]; FastSeqTrack [E14]; HiT [E18]; SeqTrack [E31].
